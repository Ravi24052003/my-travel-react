...
import { publicGetFiveInternationalItinerariesAsync, setParticularItineraryId } from "../../features/public/publicSlice";
...
const InternationalTopTourPackage = () => {
  ...
  const fiveInternationalItineraries = useSelector(state => state.public.fiveInternationalItineraries);
  ...
  useEffect(()=>{
    if(fiveInternationalItineraries?.length == 0){
      dispatch(publicGetFiveInternationalItinerariesAsync())
    }
    }, []);
  ...
  const handleNextSlide = () => {
    ...
    navigate('/all-international-itineraries');
    }
 ...
 <h1 className=" text-blue-900 text-3xl font-bold mb-2">Top most tour package</h1>
         ...
         {fiveInternationalItineraries?.map((itinerary, i) => (
